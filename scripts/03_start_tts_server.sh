...
# Print Python diagnostics for debugging the embedded interpreter
echo "[03-tts] Embedded Python diagnostics:"
"${PY_BIN}" - <<'PY'
import sys, site
print('executable:', sys.executable)
print('prefix:', sys.prefix)
print('base_prefix:', getattr(sys, 'base_prefix', ''))
print('site-packages:', [p for p in site.getsitepackages()])
...
if command -v "${TMUX_BIN}" >/dev/null 2>&1; then
  ...
  ${TMUX_BIN} new-session -d -s "${SESSION}" \
    "cd '${REPO_ROOT}' && env LD_LIBRARY_PATH='${LD_LIBRARY_PATH}' PYO3_PYTHON='${PYO3_PYTHON}' PYTHONHOME='${PYTHONHOME}' PYTHONPATH='${PYTHONPATH}' RAYON_NUM_THREADS='${RAYON_NUM_THREADS}' TOKIO_WORKER_THREADS='${TOKIO_WORKER_THREADS}' MALLOC_ARENA_MAX='${MALLOC_ARENA_MAX}' RUST_LOG='${RUST_LOG}' '${MOSHI_BIN}' worker --config '${CFG}' --addr '${ADDR}' --port '${PORT}' 2>&1 | tee '${LOG_DIR}/tts-server.log'"
else
  echo "[03-tts] tmux not found; using nohup fallback"
  ulimit -n 1048576 || true
  nohup sh -c "cd '${REPO_ROOT}' && env LD_LIBRARY_PATH='${LD_LIBRARY_PATH}' PYO3_PYTHON='${PYO3_PYTHON}' PYTHONHOME='${PYTHONHOME}' PYTHONPATH='${PYTHONPATH}' RAYON_NUM_THREADS='${RAYON_NUM_THREADS}' TOKIO_WORKER_THREADS='${TOKIO_WORKER_THREADS}' MALLOC_ARENA_MAX='${MALLOC_ARENA_MAX}' RUST_LOG='${RUST_LOG}' '${MOSHI_BIN}' worker --config '${CFG}' --addr '${ADDR}' --port '${PORT}'" \
